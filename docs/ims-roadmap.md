...
- Close remaining SIP interoperability gaps for reliable provisional responses, offer/answer edge cases, early dialogs, fork cleanup, and in-dialog routing. The opt-in worker-backed smoke now exercises reliable `183`/PRACK, in-dialog `UPDATE` offer/answer, and authenticated INVITE cancellation exchanges; external UE interoperability and the remaining early-dialog/fork cases remain open.
