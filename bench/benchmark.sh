...
RATE=${RATE:-${BENCH_TARGET_CPS:-100}}
CALLS=${CALLS:-${BENCH_CALLS:-1000}}
CONCURRENCY=${CONCURRENCY:-${BENCH_CONCURRENCY:-200}}
...
BENCH_MALLOC_MMAP_THRESHOLD=${BENCH_MALLOC_MMAP_THRESHOLD:-}
...
if [ -n "$BENCH_MALLOC_ARENA_MAX" ]; then
    export MALLOC_ARENA_MAX="$BENCH_MALLOC_ARENA_MAX"
fi
if [ -n "$BENCH_MALLOC_TRIM_THRESHOLD" ]; then
    export MALLOC_TRIM_THRESHOLD_="$BENCH_MALLOC_TRIM_THRESHOLD"
fi
if [ -n "$BENCH_MALLOC_MMAP_THRESHOLD" ]; then
    export MALLOC_MMAP_THRESHOLD_="$BENCH_MALLOC_MMAP_THRESHOLD"
fi
